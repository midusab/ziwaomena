...
const MOCK_DESTINATION: [number, number] = [-1.3, 36.83];
  ...
  return (
    <div className="h-64 w-full rounded-[32px] overflow-hidden glass border-2 border-white/20 mt-4 relative shadow-xl">
      <Map
        ...
        defaultZoom={13}
      >
        <Marker width={40} anchor={MOCK_DESTINATION} color="#000000" />

        <Marker width={45} anchor={riderPos}>
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const ordersData = snapshot.docs.map((docSnap) => ({
          ...
          ...docSnap.data(),
          createdAt:
            (docSnap.data().createdAt as Timestamp | undefined)?.toMillis() ??
            Date.now(),
        })) as Order[];
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 space-y-4">
        ...
        <p className="text-kfc-black/40 text-sm font-medium uppercase tracking-widest">
  ...
  if (orders.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-12">
        ...
        <h4 className="font-display text-2xl text-kfc-red mb-2 uppercase tracking-tight">
          No orders yet
        </h4>
        ...
        <AnimatePresence mode="popLayout">
          {displayedOrders.length === 0 ? (
            ...
            >
              ...
              <p className="text-kfc-black/30 text-sm italic">
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-8">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center ${config.bg} ${config.color} relative overflow-hidden shadow-inner`}
                        >
                          {config.icon}
                          {!isDelivered && (
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="text-[10px] uppercase font-bold text-kfc-black/40 tracking-widest">
                              #{order.id.slice(-6).toUpperCase()}
                            </p>
                            <span className="w-1 h-1 rounded-full bg-black/10" />
                            <p className="text-[10px] text-kfc-black/30">
                          ...
                          <h5 className="font-display text-2xl text-kfc-red leading-none mt-1">
                            {config.label}
                          </h5>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <p className="text-xl font-bold text-kfc-red">
                          ...
                          {[
                            ...
                            return (
                              <div key={s} className="flex flex-col items-center">
                                <div
                                  className={`w-3.5 h-3.5 rounded-full border-2 transition-all duration-700 z-10 ${
                                    isCurrent
                                      ? 'bg-kfc-red border-kfc-red scale-125 ring-4 ring-kfc-red/20'
                                      : isCompleted
                                      ? 'bg-kfc-red border-kfc-red'
                    ...
                    <div className="space-y-3">
                      <p className="text-[9px] uppercase font-bold text-kfc-black/30 px-2 tracking-widest leading-none">
                        Your Selection
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2 px-1">
                        {order.items.map((item, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-2 bg-white/40 px-3 py-2 rounded-xl border border-white/40 shadow-sm"
                          >
                            <div className="w-6 h-6 rounded-lg overflow-hidden flex-shrink-0">
                              <img
                                src={OMENA_ITEMS.find((oi) => oi.id === item.id)?.image}
                                alt=""
                                className="w-full h-full object-cover"
                                referrerPolicy="no-referrer"
                              />
                            </div>
                            <span className="text-[10px] font-bold text-kfc-black/70">
                              {item.quantity}x{' '}
                    ...
                    {!isDelivered && (
                      <div
                        className={`mt-8 p-5 rounded-[24px] ${config.bg} border border-white/20 relative overflow-hidden`}
                      >
                        ...
                        <p className="text-xs font-light leading-relaxed text-kfc-black/70 relative z-10">
                          <span className="font-bold text-kfc-red block mb-1 uppercase tracking-widest text-[9px]">
                            Live Status Update
                          </span>
                          {config.desc}
                        </p>
